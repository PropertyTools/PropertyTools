...
### Fixed
- DataGrid: Fixed `InvalidOperationException` thrown when navigating to the add-item row (or pasting rows that extend beyond the last row) in a DataView-backed DataGrid while sorting is active — `HandleAutoInsert` now checks whether row insertion succeeded before calling `GetCollectionViewIndex` #443
