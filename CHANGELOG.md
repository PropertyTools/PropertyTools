...
- PropertyGrid: Fixed error message text not refreshing for a property's error control when another property's error state changed but the tab's aggregate `HasErrors` flag stayed the same — `PropertyGridControlFactory.CreateErrorControl` now also force-refreshes the `Content` binding (not just `Visibility`) on `ErrorsChanged` #228
