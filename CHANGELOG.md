...
### Fixed
- DataGrid: Fixed infinite resizing loop when a DataGrid with star-sized columns is hosted inside a `ScrollViewer` with `Auto` scrollbars — `UpdateGridSize` now computes the available width from `sheetScrollViewer.ActualWidth` minus the width of the extra column reserved for the vertical scrollbar instead of `ViewportWidth`, which previously ignored that column and caused the column widths and viewport size to oscillate; added `ScrollViewerExample` demonstrating the scenario #176
